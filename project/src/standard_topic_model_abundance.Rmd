---
title: 'Topic model on Maps: Abundance'
author: "Kushal K Dey"
...
We read the map data into R. This map data is created by vectorizing the maps provided in the the directory `../external_data/Dispersion_Fields_with_Abundance` and then stacking these vectors in each row to get a counts data matrix of forest spots by number of cells, with the entries representing the number of distinct species of the forest spot that are present in that cell.
...
map_data <- data.frame(fread('../external_data/map_data_abundance.txt'));
...
for(k in 1:length(nclus_vec))
{
  ...
  saveRDS(Topic_clus, paste0("../rdas/topic_clus_", nclus_vec[k],"_maps_abundance.rds"));
}
```
